...
for i, j in tags:
	print(i)
```
The results are saved to a text file [node_and_tags_keys.txt](node_and_tags_keys.txt).
...
By the way, we also notice here that within values we can find ";" characters. The value should then be split and processed as a list of values.
...
When auditing key values, we noticed that some of the values given in the example were formatted like this:
`"black;yellow;black"`.
